...
def get_all_categories():
    return category_service.fetch_all_categories()


@router.get("/names", summary="Get all category names")
def get_all_category_names():
    return category_service.fetch_all_category_names()


@router.get("/count", summary="Get total number of categories")
def get_category_count():
    return {"count": category_service.fetch_total_category_count()}


@router.get("/latest", summary="Get the latest inserted category name")
def get_latest():
    latest = category_service.fetch_latest_category_name()
    if latest is None:
...
def get_category_name(category_id: int):
    try:
        return {"category_name": category_service.fetch_category_name_by_id(category_id)}
...
def get_category_id(category_name: str):
    try:
        return {"category_id": category_service.fetch_category_id_by_name(category_name)}
    except ValueError as ve:
        raise HTTPException(status_code=404, detail=str(ve))
