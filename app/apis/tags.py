...
def get_all_tag_names():
    return tag_service.fetch_all_tag_names()


@router.get("/count", summary="Get total number of tags")
def get_tag_count():
    return {"count": tag_service.fetch_total_tag_count()}


@router.get("/latest", summary="Get the latest inserted tag name")
def get_latest():
    latest = tag_service.fetch_latest_tag_name()
    if latest is None:
...
def get_tag_name(tag_id: int):
    try:
        return {"tag_name": tag_service.fetch_tag_name_by_id(tag_id)}
...
def get_tag_id(tag_name: str):
    try:
        return {"tag_id": tag_service.fetch_tag_id_by_name(tag_name)}
    except ValueError as ve:
        raise HTTPException(status_code=404, detail=str(ve))
...
def get_category(tag_name: str):
    try:
        return {"category_name": tag_service.fetch_category_name_of_tag(tag_name)}
    except ValueError as ve:
        raise HTTPException(status_code=404, detail=str(ve))
...
def upsert_tag(data: TagBase):
    try:
        tag_id = tag_service.upsert_tag(data)
