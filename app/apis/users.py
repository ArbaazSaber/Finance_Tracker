...
def update_password(user_id: int, new_password: UserPasswordUpdateRequest):
    success = users_service.update_password(user_id, new_password)
    if not success:
...
def update_last_login(user_id: int):
    success = users_service.update_last_login(user_id)
    if not success:
...
def update_email(user_id: int, new_email: str):
    success = users_service.update_email(user_id, new_email)
    if not success:
...
def delete_user(user_id: int):
    success = users_service.deactivate_user(user_id)
    if not success:
